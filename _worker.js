...
//
// معالجة أي IPv4 عام يظهر في الريدايركت (بدون تثبيت IPs):
//   المحاولة 1: اتصال مباشر عبر cloudflare:sockets (http فقط)
//   المحاولة 2: تحديث DNS لـ origin.raxson.online ثم الجلب بالاسم
...
// ============================================================
// REDIRECT FOLLOWING
// معالجة ديناميكية لأي IPv4 عام بدون تثبيت IPs في الكود
// ============================================================
...
async function followRedirects(
  ...
  while (hops < maxHops) {
    ...
    if (isIpv4Address(hostname)) {
      ...
      // ── المحاولة 1: socket مباشر (http فقط، فوري بدون DNS) ──
      if (parsed.protocol === "http:") {
        try {
          ...
          console.log("[IP SOCKET REDIRECT]", {
      ...
      // ── المحاولة 2: DNS rewrite عبر origin.raxson.online ──
      try {
        await updateOriginDns(
          hostname,
          env
        );
      } catch (error) {
        ...
        return {
          response: new Response(
            "IP fetch failed (no method worked). " +
              errors.join(" | "),
            {
              status: 502,
            }
...
function validateRedirectTarget(
  ...
  // IPv4 → مسموح، والتحقق من أنه عام يتم
  // في الدورة التالية داخل فرع isIpv4Address
...
// ============================================================
// DNS AUTO UPDATE (fallback لما يفشل الـsocket أو https)
// ============================================================

async function updateOriginDns(
  ip,
  env
) {
  if (
    ...
    throw new Error(
      "Missing CF_API_TOKEN or CF_ZONE_ID"
    );
  }

  if (
    !isPublicIpv4Address(ip)
  ) {
    throw new Error(
      "Invalid public IPv4: " + ip
    );
  }
...
async function readHttpHead(reader) {
  const marker =
    new Uint8Array([
      13,
      10,
